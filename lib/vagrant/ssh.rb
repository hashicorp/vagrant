  ...
  class SSH
    ...
    def wait_for_ssh_shell
      ...
      begin
        ...
        env.ui.info "The remote shell is NOT ready (after #{count} attempts). Report a bug if the VM to NOT halting."
        return false
      rescue => e
    ...
    def upload!(from, to)
      retryable(:tries => 5, :on => [IOError]) do
        ...
        env.logger.info("ssh") {"Running command: #{cmd}"}
    ...
    # Closes SSH processes after issuing the SSH 'exit' command to each that is alive.
    def exit_all
    ...
    def exit_and_log( name, command_timeout = 60, tries = 60)
      ...
      begin
        unless name == @control_master_command
          ...
          if alive?
            ...
            retryable(:tries => tries, :on => ChildProcess::TimeoutError, :sleep => 1.0) do
              env.ui.info( "Waiting (up to #{pause_time} sec) to exit SSH input session (pid: #{pid}) control path: #{env.config.ssh.get_control_path}...." )
          ...
          if alive?
            env.logger.info( "Failed exiting the SSH input session (pid: #{pid}) control path: #{env.config.ssh.get_control_path}." )
            kill_ssh( name )
          else
            env.ui.info( "Exited SSH input session (pid: #{pid}) control path: #{env.config.ssh.get_control_path}." ) if type == :input
            env.ui.info( "Exited SSH copy session (pid: #{pid}) control path: #{env.config.ssh.get_control_path}." ) if type == :copy
          end
        else
          if alive?
            ...
            if alive? || exists
              env.ui.info( "Killing the SSH master control session (pid: #{pid}) control path: #{env.config.ssh.get_control_path}...." )
              ::Process.kill( 'TERM', pid.to_i ) unless exists.nil?
            else
              env.ui.info( "Exited the SSH master control session (pid: #{pid}) control path: #{env.config.ssh.get_control_path}." )
            end
          end
        end
      rescue => e
        ...
        kill_ssh( name )
        env.ui.info( "ssh End polling for exit. Result #{result.inspect} Exception: #{e.inspect}\n#{caller.join('\n')}" )
      end
    end
