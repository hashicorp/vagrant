  ...
  class Machine
    ...
    def synced_folders(**opts)
      ...
      config_folders.each do |id, data|
        ...
        if impl != ""
          ...
          if !opts[:disable_usable_check]
            if !impl_class[0].new.usable?(self, true)
      ...
      if folders.key?("") && !folders[""].empty?
        default_impl = default_synced_folder_type(plugins)
        if !default_impl
      ...
      folders.each do |impl_name, fs|
        fs.each do |id, data|
          ...
          data[:impl]._initialize(self, impl_name)
        end
      end
    ...
    def default_synced_folder_type(plugins)
      ordered = []
      ...
      allowed_types = @config.vm.allowed_synced_folder_types
      if allowed_types
      ...
      ordered.each do |_, key, impl|
        return key if impl.new.usable?(self)
      end
