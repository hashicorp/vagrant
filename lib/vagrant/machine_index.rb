...
module Vagrant
  ...
  class MachineIndex
    ...
    def delete(entry)
      return true unless entry.id

      @lock.synchronize do
        with_index_lock do
          return true unless @machines[entry.id]

          # If we don't have the lock, then we need to acquire it.
          unless @machine_locks[entry.id]
            fail "Unlocked delete on machine: #{entry.id}"
          end
      ...
      @lock.synchronize do
        with_index_lock do
          ...
          unless id
            ...
            id = SecureRandom.uuid.gsub('-', '') unless id

            # Get a lock on this machine
            lock_file = lock_machine(id)
            unless lock_file
              fail "Failed to lock new machine: #{entry.name}"
            end

            @machine_locks[id] = lock_file
          end

          unless @machine_locks[id]
            fail "Unlocked write on machine: #{id}"
          end
    ...
    def unlocked_reload
      ...
      if data
        if !data['version'] || data['version'].to_i != 1
          fail Errors::CorruptMachineIndex, path: @index_file.to_s
        end
    ...
    class Entry
      ...
      # @return [Boolean]
      def valid?(home_path)
        return false unless vagrantfile_path
        return false unless vagrantfile_path.directory?
