    ...
    class SSH
      ...
      def self.check_key_permissions(key_path)
        ...
        unless stat.owned?
          ...
          fail Errors::SSHKeyBadOwner, key_path: key_path
        end
        ...
        if FileMode.from_octal(stat.mode) != '600'
          ...
          stat = key_path.stat
          if FileMode.from_octal(stat.mode) != '600'
            fail Errors::SSHKeyBadPermissions, key_path: key_path
          end
        end
      rescue Errno::EPERM
      ...
      def self.exec(ssh_info, opts = {})
        ...
        unless ssh_path
          if Platform.windows?
            fail Errors::SSHUnavailableWindows,
                 host: ssh_info[:host],
                 port: ssh_info[:port],
        ...
        host_string = "#{options[:username]}@#{host_string}" unless plain_mode
