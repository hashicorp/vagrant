...
To get started with Vagrant, download the appropriate installer or package for your platform from our [Vagrant downloads page](/vagrant/downloads). Install the package with the standard procedures for your operating system.
...
<Note heading="Rubygem installation is unsupported">

Vagrant 1.0.x has the option to be installed as a [RubyGem](https://en.wikipedia.org/wiki/RubyGems). However, this installation method is no longer supported. If you have an old version of Vagrant installed via Rubygems, remove it prior to installing newer versions of Vagrant.

</Note>

## First development environment

If you are new to Vagrant, the next step to set up a development environment is to install a [box](/vagrant/tutorials/getting-started/getting-started-boxes).
