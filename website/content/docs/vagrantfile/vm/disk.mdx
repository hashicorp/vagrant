...
- `size` (String) - The size of the disk to create. For example, `"10GB"`. Not
  ...
  More specific examples of these can be found under the provider
...
These types depend on the provider used, and may not yet be functional. Please refer to the provider specific implementation for more details for what is supported.
...
This guide is still being written as we develop this new feature for Vagrant. Is something missing, or could this be improved? Please let us know on GitHub by opening an issue or open a pull request directly.
