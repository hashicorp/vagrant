---
layout: docs
page_title: Debug and troubleshoot
description: |-
...
On Linux and Mac systems, you can prepend the `vagrant` command with an environmental variable declaration.

```shell-session
$ VAGRANT_LOG=info vagrant up
```

On Windows, you need to set the `VAGRANT_LOG` environmental variable.
...
You can also get the debug level output using the `--debug` command line option.
...
On Linux and Mac, use `&>` to redirect stderr and stdout to a file.

```shell-session
$ vagrant up --debug &> vagrant.log
```

On Windows in PowerShell, use `2>&1 | Tee-Object -FilePath ".\vagrant.log"` to print the log and redirect stderr and stdout to the log file.
