...
- `version` (string) - The expected Ansible version.
  ...
  to specify which version of Ansible must be automatically installed, but **only** in combination with the [**`install_mode`**](/docs/provisioning/ansible_local.html#install_mode) set to **`:pip`**.
