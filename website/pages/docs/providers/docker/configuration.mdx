...
- `check_usable` (boolean) - Checks is Docker is available on the host
  system. Defaults to false if `vagrant_vagrantfile` is set, otherwise
  defaults to true.
...
- `pull` (boolean) - If true, the image will be pulled on every `up` and
