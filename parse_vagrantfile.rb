...
def parse_vagrantfile(path)
  ...
  v.machine_names.each do |mach|
    ...
    # Get config.ssh.*
    plugin_configs << extract_plugin_config("ssh", root_config.ssh)

    # Get config.winrm.*
    plugin_configs << extract_plugin_config("winrm", root_config.winrm)

    #Get config.winssh.*
    plugin_configs << extract_plugin_config("winssh", root_config.winssh)
