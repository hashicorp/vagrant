  ...
  module Kernel_V2
    class VMConfig < Vagrant.plugin('2', :config)
          ...
          @provisioners.each do |p|
            if p.id
              ...
              if other_p
                ...
                other_p.run    ||= p.run
                next unless other_p.preserve_order
      ...
      def synced_folder(hostpath, guestpath, options = nil)
        ...
        options[:disabled]  = false unless options.key?(:disabled)
      ...
      def define(name, options = nil, &block)
        ...
        @__defined_vm_keys << name unless @__defined_vm_keys.include?(name)

        # Add the SubVM to the hash of defined VMs
        unless @__defined_vms[name]
      ...
      def finalize!
        ...
        @box_download_checksum = '' unless @box_download_checksum
        ...
        if @communicator == :winrm
          unless @__networks['forwarded_port-winrm']
        ...
        @provisioners.each do |p|
          p.config.finalize! unless p.invalid?
          p.run = p.run.to_sym if p.run
        end

        # If we didn't share our current directory, then do it
        # manually.
        unless @__synced_folders['/vagrant']
          synced_folder('.', '/vagrant')
        end

        @__synced_folders.each do |_id, options|
          if options[:nfs]
            options[:type] = :nfs
          end
      ...
      def get_provider_config(name)
        fail 'Must finalize first.' unless @__finalized
      ...
      def validate(machine)
        ...
        networks.each do |type, options|
          unless valid_network_types.include?(type)
