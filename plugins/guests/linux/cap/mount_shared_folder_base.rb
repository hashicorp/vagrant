...
module VagrantPlugins
  module GuestLinux
    module Cap
      class MountSharedFolderBase

        @@logger = Log4r::Logger.new("vagrant::guest::linux::#{self.name.split("::").last.downcase}")

        extend Vagrant::Util::Retryable

        def self.detect_owner_group_ids(machine, guest_path, mount_options, options)
        ...
        def self.find_mount_options_id(id_name, mount_options)
        ...
        def self.emit_upstart_notification(machine, guest_path)
