    ...
    module Action
      ...
      class HostMachineSyncFolders
        ...
        def call(env)
          ...
          unless env.key?(:host_machine_sync_folders)
        ...
        def setup_synced_folders(host_machine, env)
          ...
          our_folders.each do |type, folders|
            folders.each do |id, data|
              ...
              data[:id] = id[0...6] + rand(10_000).to_s

              # If we specify exact then we know what we're doing
              unless data[:docker__exact]
                data[:guestpath] =
                  "/var/lib/docker/docker_#{Time.now.to_i}_#{rand(100_000)}"
              end
          ...
          unless env[:host_machine_sync_folders]
            @logger.info('Not syncing folders because container created.')
            return
          end

          unless new_config.synced_folders.empty?
            # Sync the folders!
