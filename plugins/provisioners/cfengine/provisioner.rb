  ...
  module CFEngine
    class Provisioner < Vagrant.plugin('2', :provisioner)
      ...
      def cfagent_extra_args
        return '' unless @config.extra_agent_args
        " #{@config.extra_agent_args}"
      end
      ...
      def handle_cfengine_bootstrap
        ...
        unless @machine.guest.capability(:cfengine_needs_bootstrap, @config)
        ...
        unless policy_server_address
          policy_server_address = @machine.guest.capability(:read_ip_address)
          fail Vagrant::Errors::CFEngineCantAutodetectIP unless policy_server_address
        ...
        fail Vagrant::Errors::CFEngineBootstrapFailed if result != 0
      ...
      def handle_cfengine_installation
        unless @machine.guest.capability?(:cfengine_installed)
        ...
        if !installed || @config.install == :force
          fail Vagrant::Errors::CFEngineNotInstalled unless @config.install

          @machine.ui.info(I18n.t('vagrant.cfengine_installing'))
          @machine.guest.capability(:cfengine_install, @config)

          unless @machine.guest.capability(:cfengine_installed)
            fail Vagrant::Errors::CFEngineInstallFailed
          end
        end
      end
