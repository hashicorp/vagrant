    ...
    module Provisioner
      class Host < Base
        ...
        def prepare_command_arguments
          ...
          @command_arguments << "--private-key=#{@ssh_info[:private_key_path][0]}"
        ...
        def generate_inventory_machines(inventory_file)
          @machine.env.active_machines.each do |am|
            begin
              ...
              if !m_ssh_info.nil?
                inventory_file.write("#{m.name} ansible_ssh_host=#{m_ssh_info[:host]} ansible_ssh_port=#{m_ssh_info[:port]}\n")
