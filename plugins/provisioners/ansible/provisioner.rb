module VagrantPlugins
  module Ansible
    class Provisioner < Vagrant.plugin("2", :provisioner)
      ...
      # unless inventory_path is explicitly provided, and not a
      # directory.
      def setup_inventory_file
        if config.inventory_path
          inventory_file_path = Pathname.new(config.inventory_path).expand_path(@machine.env.root_path)
          return config.inventory_path if !File.directory?(inventory_file_path)
        else
          inventory_file_path = @machine.env.root_path
        end

        ssh = @machine.ssh_info

        generated_inventory_file =
          inventory_file_path.join("vagrant_ansible_inventory_#{machine.name}")
