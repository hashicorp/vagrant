...
module VagrantPlugins
  module Ansible
    class Provisioner < Vagrant.plugin("2", :provisioner)
      ...
      # based on the generated_inventory_path settings.
      # It returns the inventory_file to be used for the playbook (either the value
      # from the config.inventory_path, config.generated_inventory_file or
      # `.vagrant/provisioners/ansible/inventory` directory if nothing else specified
      #
      # The dynamic inventory file is generated even when other path is specified as the
      # inventory_path, as the generated file might still be useful even for this case.
      def setup_inventory_file
        if config.generated_inventory_path
          generated_inventory_file = Pathname.new(config.generated_inventory_path)
          generated_inventory_dir = Pathname.new(File.dirname(generated_inventory_file))
          # if the generated_inventory_file is specified, we prefer the path also
          # as the inventory path, unless specified otherwise
          inventory_file = config.inventory_path || generated_inventory_file.to_s
        else
          ...
          # to keep the backward compatibility with setting the dir as the inventory path
          # when the default generated_inventory_path is used
          inventory_file = config.inventory_path || generated_inventory_dir.to_s
        end
