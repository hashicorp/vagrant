    ...
    module Cap
      class FsISO
        ...
        def self.create_iso(env, source_directory, **extra_opts)
          ...
          source_dir_unix_path = source_directory.to_s.gsub("\\", "/")
          # If the destrination does not exist or there have been changes in the source directory since the last build, then build
          if !file_destination.exist? || Vagrant::Util::Directory.directory_changed?(source_dir_unix_path, file_destination.mtime)
