...
VF
    ...
    it "should accept parameter private_key_path" do
      ...
      env = environment.create_vagrant_env
      env.config.for_vm(:default).ssh.private_key_path.should == [tmp_file.path.to_s]
      tmp_file.close
      tmp_file.unlink
    end
    it "should accept multiple private_key_path parameters" do
      ...
      env = environment.create_vagrant_env
      env.config.for_vm(:default).ssh.private_key_path.should == [tmp_file1.path,tmp_file2.path]
    ...
    it "should accept parameter private_key_path" do
      ...
      environment = isolated_environment do |env|
        env.vagrantfile(%{Vagrant::Config.run do |config|
          config.ssh.private_key_path=["#{tmp_file1.path}","#{tmp_file2.path}"]
        end})
      end
      env = environment.create_vagrant_env
      env.config.for_vm(:default).ssh.private_key_path.should == [tmp_file1.path,tmp_file2.path]
