  ...
  describe '.configure_networks' do
    ...
    describe 'dhcp' do
      ...
      it 'plumbs the device' do
        communicator.expect_command("pfexec /sbin/ifconfig #{device} plumb")
        plugin.configure_networks(machine, [network])
      end

      it 'starts dhcp for the device' do
        communicator.expect_command("pfexec /sbin/ifconfig #{device} dhcp start")
        plugin.configure_networks(machine, [network])
      end
    end
    ...
    describe 'static' do
      ...
      it 'starts sets netmask and IP for the device' do
        communicator.expect_command("pfexec /sbin/ifconfig #{device} inet 1.1.1.1 netmask 255.255.255.0")
        plugin.configure_networks(machine, [network])
      end

      it 'starts enables the device' do
        communicator.expect_command("pfexec /sbin/ifconfig #{device} up")
        plugin.configure_networks(machine, [network])
      end
